...
def run():
   ...
   studentessays = pd.read_csv(os.path.join(con.get_filepath(), "new_data", "new_studentessay_0419.csv"), sep=',', encoding='utf_8_sig')
   ...
   for i in range(len(studentessays['论文题目'])):
       if isinstance(studentessays.iloc[i, 1], str):  # 有论文的项才处理
           ...
           title = stitle.strip()
           ori_text_filepath = os.path.join(con.get_filepath(), "new_data/StudentEssay", title + '.txt')
...
'''
           try:
           # read the essay
                PdfTranstorm(['-o', os.path.join(con.get_filepath(),"new_data/StudentEssay", title + '.txt'), '-t', 'text',
                         os.path.join(con.get_filepath(),"new_data/StudentEssay", title + '.pdf')])
           except FileNotFoundError:
                print("*******file not found***********")
                print({name:title})
                file_not_found.append({name:title})
                row = [name,title]
                out = open(os.path.join(con.get_filepath(),"new_data", 'not_found_studentessay.csv'), "a", newline="")
